...
def score_item_against_query(item: str, query_text: str) -> int:
    ...
    token_hits = sum(1 for token in q_tokens if token in item_lower)
    phrase_bonus = 2 if query_text.lower() in item_lower else 0
    return token_hits + phrase_bonus


def rank_relevant_items(items: list[str], query_text: str, limit: int = 5) -> list[str]:
    """
    Return only query-relevant items.
...
def get_relevant_evidence(row: dict[str, Any], query_text: str) -> dict[str, list[str]]:
    papers = rank_relevant_items(row["paper_titles"], query_text, limit=5)
    if not papers:
        papers = rank_relevant_items(row["paper_abstracts"], query_text, limit=5)

    courses = rank_relevant_items(row["courses"], query_text, limit=5)
    projects = rank_relevant_items(row["projects"] + row["project_descriptions"], query_text, limit=5)
...
def has_relevant_evidence(evidence: dict[str, list[str]]) -> bool:
    return bool(evidence["papers"] or evidence["courses"] or evidence["projects"])
...
def search_people(query_text: str, limit: int = MAX_RESULTS) -> list[dict[str, Any]]:
    limit = max(1, min(limit, MAX_RESULTS))
    query_type = detect_query_type(query_text)

    hits = search_index(query_text, limit=limit * 3)
